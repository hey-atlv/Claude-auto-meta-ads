...
import { ChevronUp, ChevronDown, ChevronsUpDown, TrendingUp, TrendingDown, Minus, Check, Copy } from 'lucide-react';
          ...
          <div className="overflow-x-auto">
            <table className="w-full border-collapse table-fixed" style={{ fontSize: '10px' }}>
              <colgroup>
                ...
                {p2M && <>
                  <col style={{ width: '46px' }} /><col style={{ width: '34px' }} /><col style={{ width: '50px' }} /><col style={{ width: '40px' }} />
                </>}
                {prevM && <>
                  <col style={{ width: '46px' }} /><col style={{ width: '34px' }} /><col style={{ width: '50px' }} /><col style={{ width: '40px' }} />
                </>}
              ...
              <thead>
                ...
                <tr className="bg-[#f5f6fa] border-b border-gray-100">
                  ...
                  {curM && (
                    <th colSpan={4} className="px-1 py-1 text-center text-[9px] font-bold text-orange-500 border-b-2 border-orange-400 truncate">
                      {monthLabel(curM)} ◀
                    </th>
                  )}
                  <th colSpan={1} />
                ...
                {filteredRows.map(row => {
                  ...
                  return (
                    <tr key={row.id} className={cn('border-l-[3px] hover:bg-gray-50/70 transition-colors', vs ? vs.left : 'border-l-gray-100')}>
                      ...
                      <td className={cn('px-1 py-1.5 text-right tabular-nums truncate', numCls(row.chiPhiT, medians.cp))}>{fmtCP(row.chiPhiT)}</td>
                      <td className={cn('px-1 py-1.5 text-right tabular-nums truncate', numCls(row.slDataT, medians.sl))}>{fmtSL(row.slDataT)}</td>
                      <td className={cn('px-1 py-1.5 text-right tabular-nums truncate', numCls(row.cplT, medians.cpl, true))}>{fmtCPL(row.cplT)}</td>
                      <td className={cn('px-1 py-1.5 text-right tabular-nums truncate', numCls(row.roasT, medians.roas))}>{fmtRoas(row.roasT)}</td>
